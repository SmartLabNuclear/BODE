...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 16:43:42 2020
...
#                self.add_module('norm2', nn.BatchNorm2d(out_features))
                ...
                self.add_module('upsample2',nn.Upsample(scale_factor=2, mode='bicubic'))
                self.add_module('convT2', nn.ConvTranspose2d(
                    out_features, out_features, kernel_size=3, stride=1,
                    padding=1, bias=True))
                if drop_rate > 0:
                    self.add_module('dropout2', nn.Dropout2d(p=drop_rate))
            else:
                self.add_module('upsample1',nn.Upsample(scale_factor=2, mode='bicubic'))
...
#     last_up.add_module('norm2', nn.BatchNorm2d(in_features // 2))
    ...
    last_up.add_module('upsample1',nn.Upsample(scale_factor=2, mode='bicubic'))
...
class DenseNet(nn.Module):
    def __init__(self,in_channels, out_channels, blocks, growth_rate=16,
                 init_features=64, bn_size=8, drop_rate=0, bottleneck=False,
                 out_activation=None):
...
#     dense_test = DenseNet(in_channels=4, out_channels=2, blocks=(3, 6, 3), growth_rate=16,      # in this version, only 7 input features are considered (P,T,Vx,Vy,Vz,Wall distance)
#                       init_features=32, drop_rate=0, bn_size=4,      # the only output is turbulent viscosity
#                       bottleneck=True, out_activation='ReLU')
#     print(dense_test)
#     print(dense_test._num_parameters_convlayers())
    
#     nll_loss = lambda mu, sigma, y: torch.log(sigma)/2 + ((y-mu)**2)/(2*sigma)
#     sp = torch.nn.Softplus()
    
#     x = torch.rand(8, 4, 32,64)
#     y = dense_test.forward_test(x)
    
#     mu, sig = y[:,0,:,:], sp(y[:,1,:,:]) + 1e-6 
#     output_qois = torch.rand(8,1,32,64)
#     loss = nll_loss(mu,sig, output_qois.squeeze(1))
