...
def test_bayesian(hyperparameters, processor_id):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for input_features, output_qois in train_loader:
            ...
            noise_std = 0.1
            # Generate noise, convert the tensor to a numpy array first
            ...
            # Calculate the std of the smoothed noise
            ...
            # Rescale the smoothed noise
            smooth_noise_factor_rescaled = smooth_noise_factor * rescaling_factor
            # Convert the rescaled smoothed noise from numpy array back to torch tensor
            smooth_noise_factor_tensor = torch.from_numpy(smooth_noise_factor_rescaled).to(output_qois.device)
            # Ensure smooth_noise_tensor is the same dtype as input_features
...
gs = GenerationStrategy(
    steps=[
        GenerationStep(
            ...
            num_trials=24,
            min_trials_observed=24,
        ),
        GenerationStep(
            model=Models.BOTORCH_MODULAR,
            num_trials=-1,
            # Don't override surrogate unless truly needed
        ),
    ]
)
...
def run_optimization(base_parameters, processor_id):
    ...
    ax_client.save_to_json_file(f'ax_client_state_processor_{processor_id}.json')
